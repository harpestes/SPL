...
function addProfessor(professor: Professor): void {
	...
	if (existingProfessor) {
		throw new ItemAlreadyExistsError(
			`Professor with ID ${existingProfessor.id} already exists: ${existingProfessor.name}, ${existingProfessor.department}`,
		);
	}
...
function addLesson(lesson: Lesson): boolean {
	...
	if (conflict) {
		throw new ConflictOccuredError(
			`Cannot add lesson: ${conflict.type} for lesson on ${conflict.lessonDetails.dayOfWeek} at ${conflict.lessonDetails.timeSlot}`,
		);
	}
